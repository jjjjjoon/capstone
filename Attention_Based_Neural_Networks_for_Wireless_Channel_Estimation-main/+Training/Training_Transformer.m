...
if load_parameters == true
    ...
    parameters_Transformer.Weights.decoder_layer.ln_de_w1 = initializeGlorot(sz, numOut, numIn);
    parameters_Transformer.Weights.decoder_layer.ln_de_b1 = dlarray(zeros(numFilters, 1));
    ...
    %parameters_Transformer.Weights.decoder_layer.ln_de_w1 = initializeGlorot([size(Training_Y, 1), Feature_size], prod([size(Training_Y, 1), Feature_size]), prod([size(Training_Y, 1), Feature_size]));
    %parameters_Transformer.Weights.decoder_layer.ln_de_b1 = dlarray(zeros(size(Training_Y, 1), 1));
...
for epoch = 1 : numEpochs
    ...
    while hasdata(mbqTrain)
        ...
        if iteration == 1 || mod(iteration, Validation_frequency) == 0
            ...
            % Validation set
            Prediction_validation = transformer.model_transformer(Xvalidation_minibatch, parameters_Transformer);
...
Prediction = transformer.model_transformer(X, parameters);
